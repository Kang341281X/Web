...
<template>
  <div v-if="profile" class="profile-grid" :class="{ 'single-column': profile.must_change_password }">
    ...
    <div class="profile-forms"><el-alert v-if="profile.must_change_password" type="warning" :closable="false" title="这是初始密码，请先修改密码后再使用后台其他功能。" style="margin-bottom:16px" />
      ...
      <el-card v-if="!profile.must_change_password" shadow="never" header="联系方式"><el-form :model="contact" label-width="72px"><el-form-item label="电话"><el-input v-model="contact.phone" /></el-form-item><el-form-item label="邮箱"><el-input v-model="contact.email" /></el-form-item><el-button type="primary" :loading="loading" @click="saveContact">保存联系方式</el-button></el-form></el-card>
      <el-card shadow="never" header="修改密码" :style="{ marginTop: profile.must_change_password ? '0' : '16px' }"><el-form :model="password" label-width="92px"><el-form-item label="原密码"><el-input v-model="password.current_password" type="password" show-password /></el-form-item><el-form-item label="新密码"><el-input v-model="password.new_password" type="password" show-password /></el-form-item><el-form-item label="确认新密码"><el-input v-model="password.confirm" type="password" show-password /></el-form-item><el-button type="primary" @click="savePassword">更新密码</el-button></el-form></el-card></div>
  </div>
</template>
