...
function chooseAvatar(uploadFile) { if (uploadFile.raw.size > 5 * 1024 * 1024) return ElMessage.error('图片不能超过 5MB'); avatarFile.value = uploadFile.raw; avatarPreview.value = URL.createObjectURL(uploadFile.raw); return false }
...
<template>
  <div v-if="profile" class="profile-grid"><el-card shadow="never"><template #header>个人资料</template><div class="avatar-panel"><el-avatar :size="104" :src="avatarPreview || '/assets/images/avatars/avatar-placeholder.svg'" /><el-upload :show-file-list="false" accept=".jpg,.jpeg,.png,.webp" :before-upload="chooseAvatar"><el-button>选择头像</el-button></el-upload><el-button type="primary" :disabled="!avatarFile" @click="saveAvatar">上传</el-button><small>支持 JPG、PNG、WebP，最大 5MB</small></div><el-descriptions :column="1" border><el-descriptions-item label="账号">{{ profile.username }}</el-descriptions-item><el-descriptions-item label="角色">{{ profile.role === 'super_admin' ? '超级管理员' : '管理员' }}</el-descriptions-item></el-descriptions></el-card>
