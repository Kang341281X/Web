...
async function openEdit(row) { try { const { data } = await api.get(`/products/${row.id}`); const product = data.data; Object.assign(form, product, { sku: product.sku || '', is_customizable: product.is_customizable ? '1' : '0', rating: Number(product.rating) || 5 }); images.value = data.data.images; dialogVisible.value = true } catch (error) { ElMessage.error(error.response?.data?.message || '商品详情加载失败') } }
