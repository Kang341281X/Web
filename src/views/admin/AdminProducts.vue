...
// 支持 ?stock=low URL 参数：与仪表盘「低库存预警」卡片共用后端阈值 LOW_STOCK_THRESHOLD（server/routes/products.js），
// 从仪表盘点过来即可直接看到筛好的低库存商品。
...
// 同步 URL 上的 ?stock=low：仪表盘跳转 / 浏览器前进后退 / 直接贴 URL 时都触发重新加载
watch(() => route.query.stock, value => { query.stock = value === 'low' ? 'low' : ''; query.page = 1; load() })
...
<template>
  <el-card shadow="never" class="admin-page-card admin-table-page">
    <div class="toolbar"><div class="toolbar-search"><el-input v-model="query.keyword" clearable placeholder="商品名称、SKU、生产厂家或品牌" @keyup.enter="search" @clear="search" /><el-select v-model="query.category_id" clearable placeholder="全部分类" @change="search"><el-option v-for="item in activeCategories" :key="item.id" :label="item.name" :value="item.id" /></el-select><!-- 库存等级过滤：当前仅与仪表盘共用的「低库存预警」一项；与后端 LOW_STOCK_THRESHOLD 同口径 --><el-select v-model="query.stock" clearable placeholder="全部库存" @change="search"><el-option label="低库存预警" value="low" /></el-select><el-button type="primary" @click="search">搜索</el-button><el-button v-if="query.category_id || query.stock" @click="clearFilter">清空筛选</el-button></div><div class="toolbar-actions"><el-button type="danger" :disabled="!selected.length" @click="batchRemove">批量删除</el-button><el-dropdown @command="exportProducts"><el-button>导出<el-icon class="el-icon--right"><ArrowDown /></el-icon></el-button><template #dropdown><el-dropdown-menu><el-dropdown-item command="filter">导出全部数据</el-dropdown-item><el-dropdown-item command="selected" :disabled="!selected.length">导出勾选项</el-dropdown-item></el-dropdown-menu></template></el-dropdown><el-dropdown @command="handleImportCommand"><el-button type="success">批量导入<el-icon class="el-icon--right"><ArrowDown /></el-icon></el-button><template #dropdown><el-dropdown-menu><el-dropdown-item command="online"><el-icon><Grid /></el-icon>在线表格导入</el-dropdown-item><el-dropdown-item command="excel"><el-icon><Document /></el-icon>Excel 表格导入</el-dropdown-item></el-dropdown-menu></template></el-dropdown><el-button type="primary" @click="openCreate">新增商品</el-button></div></div>
    <!-- 列宽：商品 / 分类为弹性列（min-width），两者按比例分摊剩余宽度；其余短内容列统一引用 COL 常量 -->
    <el-table v-loading="loading" :data="products" height="100%" @selection-change="handleSelection" style="width:100%"><el-table-column type="selection" :width="COL.SELECTION" /><el-table-column label="序号" :width="COL.INDEX"><template #default="{ $index }">{{ pageIndex($index) }}</template></el-table-column><el-table-column label="商品" min-width="250"><template #default="{ row }"><div class="product-cell"><el-image :src="resolve(row.main_image_url)" fit="cover"><template #error><img class="image-fallback" :src="IMG_FALLBACK" alt="" /></template></el-image><div><div>{{ row.name }}</div><small>{{ row.brand || '未设置品牌' }} · {{ row.manufacturer || '未设置厂家' }}</small><small style="display:block;margin-top:2px">SKU: {{ row.sku || '未设置' }}</small></div></div></template></el-table-column><el-table-column prop="category_name" label="分类" min-width="100" /><el-table-column label="售价" :width="COL.AMOUNT"><template #default="{ row }">¥{{ row.price.toFixed(2) }}</template></el-table-column><el-table-column label="原价" :width="COL.AMOUNT"><template #default="{ row }"><span v-if="row.original_price != null">¥{{ row.original_price.toFixed(2) }}</span><span v-else>-</span></template></el-table-column><el-table-column prop="stock" label="库存" width="64" /><el-table-column label="状态" :width="COL.STATUS_TAG"><template #default="{ row }"><el-tag :type="row.status ? 'success' : 'info'">{{ row.status ? '上架' : '下架' }}</el-tag></template></el-table-column><el-table-column label="评分" :width="COL.RATING" align="center"><template #default="{ row }">★ {{ row.rating ?? 5 }}</template></el-table-column><el-table-column label="定制" :width="COL.BOOL_TAG" align="center"><template #default="{ row }"><el-tag :type="row.is_customizable ? 'warning' : 'info'" size="small">{{ row.is_customizable ? '支持' : '不支持' }}</el-tag></template></el-table-column><el-table-column label="添加人" width="100" align="center"><template #default="{ row }">{{ row.created_by_name || '未知' }}</template></el-table-column><el-table-column label="操作" :width="actionColWidth(3)" :fixed="isMobile ? false : 'right'"><template #default="{ row }"><el-button link type="primary" @click="openEdit(row)">编辑</el-button><el-button link @click="openProduct(row)">跳转</el-button><el-button link type="danger" @click="removeProduct(row)">删除</el-button></template></el-table-column></el-table>
