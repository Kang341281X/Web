...
// 本批次是否存在由系统自动生成编号的行
const hasAutoGeneratedSku = computed(() => {
  if (!previewResult.value?.preview) return false
  return previewResult.value.preview.some(r => r.sku_auto_generated)
})
...
async function handleConfirm() {
  ...
  try {
    ...
    const replacements = data.data?.sku_replacements || []
    const countText = `成功导入 ${data.data.imported_count} 个商品`
    if (replacements.length) {
      ElMessage.warning(`${countText}；其中 ${replacements.length} 个商品编号在预览后已被占用，系统已自动更换为新的唯一编号（详见后台操作日志）`)
    } else {
      ElMessage.success(countText)
    }
    emit('success')
    resetAndClose()
  } catch (error) {
...
<template>
  ...
  >
        ...
        <template #title>
          <div class="alert-content">
            ...
            <ol>
              ...
              <li>「商品编号(SKU)」选填，留空会在预览阶段自动生成并展示唯一编号（带「自动生成」标签）；编号确认导入后不可修改，重复（含已存在）则判定失败；「是否支持定制」可用下拉选"是/否"；「商品评分」留空默认为 5</li>
    ...
    <div v-if="step === 2 && previewResult" class="import-step">
      ...
      <el-alert
        v-if="hasAutoGeneratedSku"
        type="info"
        :closable="false"
        show-icon
        style="margin-bottom: 12px"
      >
        <template #title>未填写商品编号(SKU)的行，系统已在此自动生成唯一编号（带「自动生成」标签），确认导入时将原样写入且此后不可修改；如需更换请删除该商品后重新新增</template>
      </el-alert>
      ...
      >
        ...
        <el-table-column label="商品编号(SKU)" min-width="170" show-overflow-tooltip>
          <template #default="{ row }">
            <span>{{ row.sku || '-' }}</span>
            <el-tag v-if="row.sku_auto_generated" size="small" type="info" style="margin-left: 4px">自动生成</el-tag>
          </template>
        </el-table-column>
