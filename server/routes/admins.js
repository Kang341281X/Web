...
router.post('/', async (req, res, next) => {
  try {
    ...
    await writeOperationLog(req.admin.id, 'create_admin', username)
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const [updated] = await db.execute(`SELECT ${fields} FROM admin WHERE id = ?`, [target.id]); await writeOperationLog(req.admin.id, 'update_admin', target.username)
...
router.put('/:id/reset-password', async (req, res, next) => {
  try {
    ...
    await db.execute('UPDATE admin SET password = ?, must_change_password = 1 WHERE id = ?', [await bcrypt.hash(password, 12), target.id]); await writeOperationLog(req.admin.id, 'reset_admin_password', target.username)
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    await db.execute('DELETE FROM admin WHERE id = ?', [target.id]); await writeOperationLog(req.admin.id, 'delete_admin', target.username)
